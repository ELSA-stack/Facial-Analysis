...
import { GENERIC_BASELINE } from '../../vision/constants'
...
export default function MemeScoreView() {
  ...
  const processFrame = useCallback(() => {
    ...
    if (detector && detector.isInitialized) {
      ...
      if (isScanning && scanStartTimeRef.current) {
        ...
        if (elapsed < DURATION) {
          ...
          const m = face ? measure(face, GENERIC_BASELINE) : {}
          const tongue = face ? tongueScore(ctx, face, hands, m.z_jaw >= 3.5 && m.jaw >= 0.18) : 0.0
          const gesture = motionRef.current.update(hands, face)

          const features = extractAllFeatures(face, hands, body, tongue, motionRef.current, GENERIC_BASELINE)
          const rxResult = decide(face, hands, body, tongue, gesture, m, features, GENERIC_BASELINE)
    ...
    animFrameIdRef.current = requestAnimationFrame(processFrame)
  }, [isStreaming, isScanning, stopCamera])
